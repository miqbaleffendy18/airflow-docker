...
from utils.etl import database_stream, database_extract
...
get_name = lambda: os.environ.get("name") or 'EVERPRO'
get_ssl_mode = lambda: os.environ.get("ssl_mode") or 'require'
    ...
    for index, row in df_init.iterrows():
        try:
            ...
            source_credential = {
                ...
                'additional': f'sslmode={get_ssl_mode()}'
            }

            list_column = get_target_column(snow_credential=snow_credential, db=db_target, schema=schema_target, table=table_target, dbtype='postgresql')
...
if __name__ == '__main__':
    data_split = np.array_split(df_init, 3)
    with Pool(3) as p:
